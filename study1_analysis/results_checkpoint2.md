...
Every category model fit cleanly with the prereg-specified random-effects structure; no fallback to a reduced spec was needed. The trial-level random intercept estimated to zero in all three category-model families (H1-PRO, H1-IDM, H2-AFF) in both datasets — consistent with the convergence-risk forecast in `notebooks/README.md`, which flagged IDM as the most likely failure point because 78.9% of trials have zero IDM words. Within the fitted structure, a variance component at zero is mathematically equivalent to fitting without that term, so fixed-effect coefficients and LRT statistics are unaffected. One additional case: in H1-PRO on the primary dataset the `model_id` variance component also estimated to zero, indicating that the family fixed effect and the family × framing interaction absorbed the available between-model variance in that specific fit; the LRT remains valid because both full and reduced models fit. Variance-component values for all category and H4 fits are in `notebooks/output/variance_components.txt`.
...
- `study1_analysis/notebooks/02_variance_components.R` — extracts variance components from the saved model objects.
- `study1_analysis/notebooks/output/primary_results.csv` — flat result table for the 5 tests × 2 datasets.
- `study1_analysis/notebooks/output/primary_models.rds` — saved fitted model objects.
- `study1_analysis/notebooks/output/variance_components.txt` — variance components for each random-effects term in each fit.
